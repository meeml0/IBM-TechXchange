...
import { UnilabBlogContent } from '@/app/types/unilabBlog';

// Desteklenen diller ve sayfalar için tip tanımları
type SupportedLocale = 'tr' | 'en';
type PageType = 'about' | 'projects' | 'blog' | 'services' | 'careers' | 'contact' | 'terms' | 'privacy' | 'newsletter' | 'not-found';
...
const bgImages: Record<PageType, string> = {
  ...
  'terms': '/terms-hero.webp',
  'privacy': '/privacy-hero.webp',
  'newsletter': '/newsletter-hero.webp',
...
export default async function PageHandler({ pageType, locale }: PageHandlerProps) {
  ...
  const validPageType = ['about', 'projects', 'blog', 'services', 'careers', 'contact', 'terms', 'privacy', 'newsletter'].includes(pageType) 
  ...
  if (validPageType === 'about') {
    ...
    const blogContent = await getPageContent<UnilabBlogContent>(locale, validPageType);
    if (blogContent) {
      content = blogContent;
    ...
    content = {
      ...
      description: locale === 'tr' 
        ? 'UNILAB Vision hizmetlerinin kullanım koşulları ve şartları.'
        : 'Terms and conditions for using UNILAB Vision services.'
    };
