...
const generateConfirmationEmail = (firstName: string, lastName: string, submissionId: string, locale: string) => {
  const content = {
    tr: {
      ...
      contactInfo: `info@unilabvision.com<br>Mesaj No: ${submissionId}`,
    ...
    en: {
      ...
      contactInfo: `info@unilabvision.com<br>Message No: ${submissionId}`,
...
const generateNotificationEmail = (
  ...
  const content = {
    tr: {
      ...
      footerText: 'UNILAB Vision - Otomatik Bildirim Sistemi',
      submissionNo: 'Mesaj No:',
      date: 'Tarih:',
      name: 'Ad Soyad:',
      emailLabel: 'E-posta:',
    ...
    en: {
      ...
      footerText: 'UNILAB Vision - Automatic Notification System',
      submissionNo: 'Message No:',
      date: 'Date:',
      name: 'Name:',
      emailLabel: 'Email:',
...
export async function POST(request: NextRequest) {
  try {
    ...
    const totalNotifications = (process.env.NOTIFICATION_EMAILS?.split(',') || ['info@unilabvision.com']).length;

    // Send confirmation email to user
    try {
      await sendEmail(
        email,
        locale === 'tr' ? 'Mesajınızı Aldık - UNILAB Vision' : 'We Received Your Message - UNILAB Vision',
    ...
    const notificationEmails = process.env.NOTIFICATION_EMAILS?.split(',') || ['info@unilabvision.com'];
    ...
    for (const adminEmail of notificationEmails) {
      ...
      try {
        await sendEmail(
          cleanEmail,
          locale === 'tr' ? 'Yeni İletişim Mesajı - UNILAB Vision' : 'New Contact Message - UNILAB Vision',
